...
# Create singleton clients (e.g., during app startup)
sync_client = Client(host=settings.OLLAMA_HOST)  # e.g., "http://localhost:11434"
async_client = AsyncClient(host=settings.OLLAMA_HOST)
...
def chat_stream(messages, model: str = None):
    ...
    stream = sync_client.chat(model=model, messages=messages, stream=True)
    t1 = time.time()
...
async def async_chat_stream(messages, model: str = None):
    """
    Truly async version of chat_stream using AsyncClient
    """
    model = model or settings.OLLAMA_MODEL
    t0 = time.time()
    print(f"[async_chat_stream] START model={model}; building request...")

    # Kick off Ollama request in streaming mode
    stream = await async_client.chat(model=model, messages=messages, stream=True)
    t1 = time.time()
    print(f"[async_chat_stream] REQUEST SENT in {t1 - t0:.2f}s; waiting for first token...")
    ...
    async for chunk in stream:
        ...
        if not got_first:
            print(f"[async_chat_stream] FIRST TOKEN after {current_time - t1:.2f}s (total {current_time - t0:.2f}s)")
            got_first = True
        ...
        if piece:  # Only count and log non-empty pieces
            ...
            # Log every 5 tokens or for the first few tokens
            if token_count <= 10 or token_count % 5 == 0:
                print(f"[async_chat_stream] Token #{token_count}: {len(piece)} chars, "
                      f"time_since_last: {time_since_last_token:.3f}s, "
                      f"total_time: {current_time - t0:.2f}s")
            elif piece.endswith(('.', '!', '?', '\n')):  # Log at sentence endings
                print(f"[async_chat_stream] Token #{token_count}: {len(piece)} chars, "
    ...
    if token_count > 0:
        avg_time_per_token = (time.time() - t1) / token_count
        print(f"[async_chat_stream] STREAM END - Total tokens: {token_count}, "
              f"Total time: {total_time:.2f}s, "
              f"Avg time/token: {avg_time_per_token:.3f}s")
    else:
        print(f"[async_chat_stream] STREAM END - No tokens received, Total time: {total_time:.2f}s")